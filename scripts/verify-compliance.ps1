...
$allCsFiles = Get-ChildItem -Path @((Join-Path $RootDirectory "src"), (Join-Path $RootDirectory "tests"), (Join-Path $RootDirectory "benchmarks"), (Join-Path $RootDirectory "samples")) -Recurse -Filter "*.cs" -ErrorAction SilentlyContinue | Where-Object { $_.FullName -notmatch '[\\/](obj|bin)[\\/]' }
$missingHeaders = 0
...
if ($missingHeaders -eq 0) { Write-Host "  ✅ All C# source files (src, tests, benchmarks, samples) contain the required MIT copyright header." -ForegroundColor Green }

# 4. One Type Per File in src/ and tests/
Write-Host "`n[4/7] Checking 'One Type Per File' rule in src/ and tests/..." -ForegroundColor Yellow
$multiTypeFiles = 0
$checkedCsFiles = Get-ChildItem -Path @((Join-Path $RootDirectory "src"), (Join-Path $RootDirectory "tests")) -Recurse -Filter "*.cs" -ErrorAction SilentlyContinue | Where-Object { $_.FullName -notmatch '[\\/](obj|bin)[\\/]' }
foreach ($cs in $checkedCsFiles) {
...
if ($multiTypeFiles -eq 0) { Write-Host "  ✅ Every source and test file satisfies the 'One Type Per File' invariant." -ForegroundColor Green }
...
# 7. Prohibited <NoWarn> Compiler Suppressions (Zero Tolerance for CS1591, CS0618, CS0619, CS159)
Write-Host "`n[7/7] Checking for prohibited compiler warning suppressions (<NoWarn>)..." -ForegroundColor Yellow
$projFiles = Get-ChildItem -Path $RootDirectory -Recurse -Filter "*.*proj" -ErrorAction SilentlyContinue | Where-Object { $_.FullName -notmatch '[\\/](obj|bin)[\\/]' }
$prohibitedWarns = @("CS1591", "CS0618", "CS0619", "CS159", "CS0159", "1591")
$badWarns = 0
foreach ($pf in $projFiles) {
    $pContent = Get-Content $pf.FullName -Raw
    foreach ($pw in $prohibitedWarns) {
        if ($pContent -match "<NoWarn>[^<]*\b$pw\b") {
            Write-Host "  ❌ Prohibited <NoWarn> suppression '$pw' detected in $($pf.FullName)" -ForegroundColor Red
            $violations++
            $badWarns++
        }
    }
}
if ($badWarns -eq 0) { Write-Host "  ✅ Zero prohibited compiler warning suppressions (CS1591, CS0618, CS0619, CS159)." -ForegroundColor Green }

# ImplicitUsings Global Policy Gate
Write-Host "`n[Gate: ImplicitUsings] Validating ImplicitUsings policy..." -ForegroundColor Yellow
$targetRootForBld = if (Get-Variable -Name "RootDirectory" -Scope 0 -ErrorAction SilentlyContinue) { $RootDirectory } else { "." }
$bldPropsPath = Join-Path $targetRootForBld "Directory.Build.props"
...
if ($bldContent -notmatch '<ImplicitUsings>\s*disable\s*</ImplicitUsings>') {
    ...
    Write-Host "  ✅ ImplicitUsings disabled globally in Directory.Build.props." -ForegroundColor Green
}
