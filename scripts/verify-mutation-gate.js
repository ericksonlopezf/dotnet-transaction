...
async function verifyMutationGate({ github, context, core } = {}) {
  const owner = context?.repo?.owner || (process.env.GITHUB_REPOSITORY ? process.env.GITHUB_REPOSITORY.split('/')[0] : 'ericksonlopezf');
  const repo = context?.repo?.repo || (process.env.GITHUB_REPOSITORY ? process.env.GITHUB_REPOSITORY.split('/')[1] : 'dotnet-transaction');
  const targetSha = process.env.TARGET_SHA || context?.sha || process.env.GITHUB_SHA || 'unknown';
  ...
  // 1. First inspect direct targetSha commit status
  if (github?.rest?.repos?.getCombinedStatusForRef && targetSha && targetSha !== 'unknown') {
    try {
      ...
      const sStatus = (cStatusResp.data?.statuses || []).find(
        s => s.context === 'mutation-testing/stryker' || s.context === 'quality-gate/stryker-mutation' || s.context === 'stryker/mutation-score' || s.context === 'stryker/mutation-gate'
      );
      ...
      if (sStatus) {
        ...
        executionDate = sStatus.updated_at || sStatus.created_at;
        runUrl = sStatus.target_url;
        mutationScore = parseScoreFromDescription(statusDescription);
        evaluationSource = `direct_commit_status (${targetSha.substring(0, 7)})`;
      }
    } catch (err) {
      console.log(`[INFO] Direct commit status check for target commit skipped or failed: ${err.message}`);
    }
  }

  // 2. If not found directly, search recent commits on 'main' (up to 20 commits) for Stryker commit status
  if (!evaluatedCommit && github?.rest?.repos?.listCommits && github?.rest?.repos?.getCombinedStatusForRef) {
  ...
  // 3. If not found via commit status, query completed workflow runs of mutation-testing.yml strictly on main
  if (!evaluatedCommit && github?.rest?.actions?.listWorkflowRuns) {
  ...
  // 4. Fallback for local testing or same-workspace runs
  if (!evaluatedCommit && fs.existsSync('StrykerOutput/aggregate-summary.json')) {
    try {
      const summary = JSON.parse(fs.readFileSync('StrykerOutput/aggregate-summary.json', 'utf8'));
      evaluatedCommit = summary.commit_sha || targetSha;
      statusState = summary.passed_break ? 'success' : 'failure';
      executionDate = summary.execution_date || new Date().toISOString();
      mutationScore = Number(summary.overall_mutation_score || 0);
      runUrl = summary.run_url || '';
      evaluationSource = 'local_aggregate_summary';
      console.log(`[INFO] Found local Stryker aggregate summary.`);
    } catch (e) {
      console.warn(`[WARN] Could not read local aggregate summary: ${e.message}`);
    }
  }
...
> [!NOTE]
> Mutation testing is running conditionally. If all 11 packages achieve $\\ge ${thresholds.break}\\%$, publication will proceed automatically.
`;
  } else {
...
| Audit Item | Value |
|---|---|
| **Evaluated Commit SHA (main)** | \`${evaluatedCommit ? evaluatedCommit.substring(0, 7) : targetSha.substring(0, 7)}\` |
...
`;
  ...
  if (core && core.summary && typeof core.summary.addRaw === 'function') {
    await core.summary.addRaw(summary).write();
  } else if (stepSummaryPath && fs.existsSync(path.dirname(stepSummaryPath))) {
    fs.appendFileSync(stepSummaryPath, summary, 'utf8');
  }
