...
public sealed class TransactionContextTests
{
    ...
    [InlineData("   ")]
    [InlineData(null)]
    public async Task CreateSavepointAsync_WhenNameNullOrWhitespace_ShouldThrowArgumentException(string? name)
    {
        var dbConn = Substitute.For<DbConnection>();
        var dbTx = Substitute.For<DbTransaction>();
        var sm = new TransactionStateMachine();
    ...
    [InlineData("sp;semi")]
    [InlineData("sp!excl")]
    public async Task CreateSavepointAsync_WhenNameContainsInvalidCharacters_ShouldThrowArgumentException(string name)
    {
        var dbConn = Substitute.For<DbConnection>();
        var dbTx = Substitute.For<DbTransaction>();
        var sm = new TransactionStateMachine();
    ...
    [Fact]
    public async Task CreateSavepointAsync_WhenNameInvalid_ShouldNotExecuteDatabaseCommand()
    {
        var dbConn = Substitute.For<DbConnection>();
        var dbTx = Substitute.For<DbTransaction>();
        var sm = new TransactionStateMachine();
        ...
        await act.Should().ThrowAsync<ArgumentException>();
        await dbTx.DidNotReceiveWithAnyArgs().SaveAsync(default!, default);
        dbConn.DidNotReceiveWithAnyArgs().CreateCommand();
        await context.DisposeAsync();
    }
