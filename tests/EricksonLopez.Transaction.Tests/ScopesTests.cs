...
public sealed class ScopesTests
{
    ...
    private sealed class ThrowingDisposeTransaction : ITransaction
    {
        ...
        public ITransactionContext Context => null!;
    ...
    [Fact]
    public async Task AmbientTransactionScope_WhenInnerDisposeThrows_ShouldStillRestoreAmbientContext()
    {
        var holder = new AsyncLocal<ITransactionContext?>();
        var previousContext = Substitute.For<ITransactionContext>();
        var innerTx = new ThrowingDisposeTransaction();
    ...
    public async Task SuppressedTransactionScope_CommitAndDispose_ShouldWorkCorrectly()
    {
        ...
        // Second dispose is no-op and must not overwrite holder.Value
        var otherContext = Substitute.For<ITransactionContext>();
        holder.Value = otherContext;
        await scope.DisposeAsync();
        holder.Value.Should().BeSameAs(otherContext);
