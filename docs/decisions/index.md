...
| [**ADR-023**](../adr/adr-023-rejection-of-infrastructure-scope-expansion.md) | Rejection of Infrastructure Scope Expansion | **Accepted (Rejection)** | ORM Change Tracking, Pooling, Caching rejected (Section 4 superseded by ADR-027). |
| [**ADR-024**](../adr/adr-024-transaction-mediator-deferred-to-separate-package.md) | Transaction.Mediator — First-Class Ecosystem Package | **Accepted** | In-repo mediator command pipeline behavior and declarative transactional boundaries. |
...
| [**ADR-027**](../adr/adr-027-entity-framework-core-transaction-enlistment.md) | Entity Framework Core Transaction Enlistment Bridge | **Accepted** | Frictionless `UseTransactionAsync` bridge for combining EF Core with ambient transaction contexts. |
| [**ADR-028**](../adr/adr-028-polly-resilience-ambiguous-commit-handling.md) | Polly Resilience Extensions for Ambiguous Commit Handling | **Accepted** | Strongly-typed `HandleAmbiguousCommit` policy extensions for idempotency and transient retries. |
| [**ADR-029**](../adr/adr-029-roslyn-analyzer-connection-manipulation.md) | Roslyn Diagnostic Analyzer for Direct Connection Manipulation | **Accepted** | Compile-time rule `ELT001` preventing manual closing or disposing of `context.Connection`. |
| [**ADR-030**](../adr/adr-030-pluggable-database-dialect-abstraction.md) | Pluggable Database Dialect Abstraction | **Accepted** | `IDatabaseDialect` abstraction and `DialectResolver` for multi-engine savepoint SQL syntax. |
| [**ADR-031**](../adr/adr-031-pluggable-native-oracle-dialect-specification.md) | Pluggable Native Oracle Dialect Specification | **Accepted** | `OracleDialect` implementation with `SET TRANSACTION READ ONLY;` and safe savepoint omission. |
| [**ADR-032**](../adr/adr-032-transactional-attribute-metadata-marker-no-runtime-effect.md) | TransactionalAttribute as Metadata Marker | **Accepted** | `[Transactional]` has no runtime effect in the pipeline. `ITransactionalCommandOptions` is the sole functional mechanism. AOT-compliant design rationale. |
| [**ADR-033**](../adr/adr-033-sanitize-telemetry-metadata-pii-redaction-scope.md) | SanitizeTelemetryMetadata PII Redaction Scope | **Accepted** | `TransactionOptions.SanitizeTelemetryMetadata` redacts `transaction.id` and `transaction.name` span tags. SQL parameter redaction is out of scope. |
